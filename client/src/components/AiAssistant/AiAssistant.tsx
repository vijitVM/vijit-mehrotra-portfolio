...
const EnhancedMarkdown: React.FC<{ content: string }> = ({ content }) => {
  ...
  lines.forEach((line, index) => {
    ...
    if (!line.trim().startsWith('*') && !line.trim().startsWith('-') && !/\d+\./.test(line.trim().substring(0, 3))) {
      inList = false;
    }
    ...
    if (line.startsWith('# ')) {
      ...
      elements.push(<h2 key={index} className="text-lg font-semibold mt-3 mb-1">{line.substring(3)}</h2>);
    } else if (line.startsWith('### ')) {
      elements.push(<h3 key={index} className="text-md font-semibold mt-2">{line.substring(4)}</h3>);
    } else if (line.trim().startsWith('*') || line.trim().startsWith('-') || /\d+\./.test(line.trim().substring(0, 3))) {
      const listType = (line.trim().startsWith('*') || line.trim().startsWith('-')) ? 'ul' : 'ol';
      ...
      if (!inList) {
        elements.push(React.createElement(listType, { key: `list-${index}`, className: listType === 'ul' ? 'list-disc list-inside' : 'list-decimal list-inside' }, [listItem]));
        inList = true;
      } else {
...
${jobDescription}`;
  ...
  const InitialView = () => (
    <div className="text-center text-gray-500 p-4">
      <h4 className="font-semibold text-gray-700 mb-3">Welcome! How can I help?</h4>
      <p className="text-sm mb-4">I can answer questions about Vijit's portfolio or analyze a job description for you.</p>
      <div className="flex flex-col gap-2 text-sm">
        <button onClick={() => handleSendMessage('What are his key technical skills?')} className="p-2 bg-gray-200 rounded-lg hover:bg-gray-300 transition-colors">
          What are his key technical skills?
        </button>
        <button onClick={() => handleSendMessage('Tell me about the VOC Complaint Analyzer project.')} className="p-2 bg-gray-200 rounded-lg hover:bg-gray-300 transition-colors">
          Tell me about a key project.
        </button>
        <button onClick={() => setAnalysisMode(true)} className="p-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors font-semibold">
  ...
  const AnalysisView = () => (
    <div className="flex flex-col h-full p-4">
        <h3 className="font-bold text-lg text-gray-800">Candidate Fit Analysis</h3>
        <p className="text-sm text-gray-600 mb-3">Paste a job description below to generate a detailed "Candidate Fit Report".</p>
        <textarea
            value={jobDescription}
            onChange={(e) => setJobDescription(e.target.value)}
            placeholder="Paste the full job description here... (min. 50 characters)"
            className="flex-1 w-full p-2 border rounded-md resize-none bg-gray-50"
            disabled={isLoading}
        />
        <div className="mt-3 flex gap-2">
            <button onClick={() => setAnalysisMode(false)} className="px-4 py-2 bg-gray-200 rounded-md hover:bg-gray-300 transition-colors">
                Back
            </button>
            <button 
                onClick={handleAnalyzeJobFit} 
                disabled={isLoading || jobDescription.length < 50} 
                className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-md disabled:bg-blue-400 hover:bg-blue-700 transition-colors">
      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-80 sm:w-96 bg-white rounded-lg shadow-2xl flex flex-col" style={{ height: '32rem' }}>
           {analysisMode ? <AnalysisView /> : (
            <>
              <div className="p-4 bg-gray-100 rounded-t-lg"><h3 className="font-bold text-lg text-gray-800">Portfolio Assistant</h3><p className="text-sm text-gray-600">Q&A and Candidate Fit Analysis</p></div>
              <div ref={chatBoxRef} className="flex-1 p-4 overflow-y-auto space-y-4">
                ...
                {messages.map((msg, index) => (
                  <div key={index} className={`flex items-end gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`p-3 rounded-xl max-w-xs md:max-w-md break-words ${msg.role === 'user' ? 'bg-blue-500 text-white rounded-br-none' : 'bg-gray-200 text-gray-800 rounded-bl-none'}`}>
